pipeline {
    agent { label 'ecs' }
    parameters { 
        string(name: 'IMAGE_TAG', defaultValue: '', description: 'This is the OCU API tag in the repo') 
    ...
    environment { 
        ...
        SERVICE = "oculus-cdk-${params.ENV}-api"
        TASK_DEF = "oculus-cdk-${params.ENV}-api"
        FAMILY = "oculus-cdk-${params.ENV}-api"
    }
    stages {
        ...
        stage('Update App Task') {
            steps {
                echo 'Updating Task..'
                sh """
                    sed -i "s/<<IMAGEID>>/${params.IMAGE_TAG}/g" task-defs/api/taskdef-${params.ENV}-cdk.json
                    sed -i "s/<<RELIC_ENABLE>>/${params.RELIC_ENABLE}/g" task-defs/api/taskdef-${params.ENV}-cdk.json                
                """
                script{
                    REVISION= sh(returnStdout: true, script: """aws ecs register-task-definition --region \$AWS_REGION --family \$FAMILY --cli-input-json file://task-defs/api/taskdef-${params.ENV}-cdk.json| jq .taskDefinition.revision""")
                }
