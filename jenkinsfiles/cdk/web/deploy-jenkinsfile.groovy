pipeline {
    agent { label 'ecs' }
    parameters { 
        string(name: 'IMAGE_TAG', defaultValue: '', description: 'This is the OCU web tag in the repo') 
        ...
        choice(name: 'RELIC_ENABLE', choices: ['false', 'true'], description: 'Enable New Relic APM?  There is a cost associated to this.')
    }
    environment { 
        GIT_REPO = 'ssh://git@bitbucket.pearson.com/cite-ocu/oculus-devops.git'
        ECR_ACCOUNT = '346147488134'
        ECR_REPO = 'oculus-web'
        ECS_CLUSTER = "oculus-cdk-${params.ENV}"
        AWS_REGION = 'us-east-1'
        SERVICE = "oculus-cdk-${params.ENV}-web"
        TASK_DEF = "oculus-cdk-${params.ENV}-web"
        FAMILY = "oculus-cdk-${params.ENV}-web"
    }
    stages {
        ...
        stage('Update App Task') {
            steps {
                echo 'Updating Task..'
                sh """
                    sed -i "s/<<IMAGEID>>/${params.IMAGE_TAG}/g" task-defs/web/taskdef-${params.ENV}-cdk.json
                    sed -i "s/<<RELIC_ENABLE>>/${params.RELIC_ENABLE}/g" task-defs/web/taskdef-${params.ENV}-cdk.json
                """
                script{
                    REVISION= sh(returnStdout: true, script: """aws ecs register-task-definition --region \$AWS_REGION --family \$FAMILY --cli-input-json file://task-defs/web/taskdef-${params.ENV}-cdk.json| jq .taskDefinition.revision""")
                }
