pipeline {
    ...
    environment { 
        ...
        SERVICE = "oculus-cdk-${params.ENV}-tca"
        TASK_DEF = "oculus-cdk-${params.ENV}-tca"
        FAMILY = "oculus-cdk-${params.ENV}-tca"
    }
    stages {
        ...
        stage('Update App Task') {
            steps {
                echo 'Updating Task..'
                sh """
                    sed -i "s/<<IMAGEID>>/${params.IMAGE_TAG}/g" task-defs/tca-api/taskdef-${params.ENV}-cdk.json
                    sed -i "s/<<RELIC_ENABLE>>/${params.RELIC_ENABLE}/g" task-defs/tca-api/taskdef-${params.ENV}-cdk.json                
                """
                script{
                    REVISION= sh(returnStdout: true, script: """aws ecs register-task-definition --region \$AWS_REGION --family \$FAMILY --cli-input-json file://task-defs/tca-api/taskdef-${params.ENV}-cdk.json| jq .taskDefinition.revision""")
                }
