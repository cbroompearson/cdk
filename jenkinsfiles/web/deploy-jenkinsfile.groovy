pipeline {
    ...
    environment { 
        ...
        ECR_REPO = 'oculus-web'
        ECS_CLUSTER = "ocu-${params.ENV}-ecs"
        AWS_REGION = 'us-east-1'
        SERVICE = "ocu-${params.ENV}-web"
        TASK_DEF = "ocu-${params.ENV}-web"
        FAMILY = "ocu-${params.ENV}-web"
    }
    stages {
        ...
        stage('Update App Task') {
            steps {
                echo 'Updating Task..'
                sh """
                    sed -i "s/<<IMAGEID>>/${params.IMAGE_TAG}/g" task-defs/web/taskdef-${params.ENV}.json
                    sed -i "s/<<RELIC_ENABLE>>/${params.RELIC_ENABLE}/g" task-defs/web/taskdef-${params.ENV}.json
                """
                script{
                    REVISION= sh(returnStdout: true, script: """aws ecs register-task-definition --region \$AWS_REGION --family \$FAMILY --cli-input-json file://task-defs/web/taskdef-${params.ENV}.json| jq .taskDefinition.revision""")
                }
