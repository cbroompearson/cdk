    ...
    stages {
        ...
        stage('Update App Task') {
            steps {
                echo 'Updating Task..'
                sh """
                sed -i "s/<<IMAGEID>>/${params.IMAGE_TAG}/g" task-defs/tca-api/taskdef-${params.ENV}.json
                sed -i "s/<<RELIC_ENABLE>>/${params.RELIC_ENABLE}/g" task-defs/api/taskdef-${params.ENV}.json  
                """
                script{
                    REVISION= sh(returnStdout: true, script: """aws ecs register-task-definition --region \$AWS_REGION --family \$FAMILY --cli-input-json file://task-defs/tca-api/taskdef-${params.ENV}.json| jq .taskDefinition.revision""")
                }
